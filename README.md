...
- скрытие плейлистов: спрятанные плейлисты не показываются на главной и прячутся под кат на странице плейлистов
...
- три источника текста: lrclib, genius и яндекс
- режим "авто" берёт сначала lrclib, затем genius, яндекс подключается вручную
...
- разборы строчек, авторы и продюсеры с genius.com
...
один участник ведёт (хост), остальные повторяют его трек, паузу и позицию.
всё находится в настройках, раздел "слушать вместе". есть два варианта связи:
**через сервер** и **через локалку**. переключатель типа связи там же, вверху
раздела. менять его можно только пока вы не в комнате.

#### вариант 1: через сервер (простой)

участники обмениваются кодом из шести слов.

1. настройки → "слушать вместе", тип связи - "сервер"
2. в поле "сервер" уже подставлен общий сервер по умолчанию -
   `mashiro.onecorporation.cfd`, можно оставить как есть
3. хост жмёт "создать комнату" и получает код вида
   `olive canyon spirit velvet ladder onion`
4. код пересылается друзьям, они вставляют его в "войти по коду" и жмут
   "войти"
5. выйти можно кнопкой "выйти" там же

подробности:

- адрес сервера запоминается, в поле можно вписать свой домен.
- ключ-аккаунт создаётся автоматически и хранится на устройстве; в блоке
  "дополнительное" его можно посмотреть, ввести старый или сбросить

сервер можно поднять у себя - исходники и инструкция лежат в отдельном
репозитории: [elytrya/mashiro-together](https://github.com/elytrya/mashiro-together). 
после запуска просто впишите свой адрес вместо дефолтного - все участники должны указать один
и тот же сервер.

#### вариант 2: через локалку

комната поднимается прямо в клиенте хоста, остальные подключаются к его ip.
подходит для одной квартиры/офиса или любого vpn с общей подсетью.

1. настройки → "слушать вместе", тип связи - "локалка"
2. хост выбирает порт (по умолчанию 7331) и жмёт "создать" в блоке "своя
   комната"
3. появляется "адрес для друзей" вида `26.13.4.7:7331` - его можно скрыть
   глазом или скопировать одним кликом
4. остальные вставляют этот адрес в "подключиться к комнате"
5. хост закрывает комнату кнопкой "закрыть", гость - "отключиться"

подробности:

- подходят radmin vpn, hamachi, zerotier, tailscale, netbird и любой другой
  туннель, где участники видят ip друг друга; адрес берётся из окна самой
  программы, порт тот же
- через голый интернет без vpn нужен проброс порта на роутере и разрешение
  в фаерволле - если лень, берите вариант с сервером

#### что работает в любом из вариантов

- один участник ведёт, остальные повторяют трек, паузу, перемотку и очередь
- общая загрузка: пока у кого-то трек ещё грузится, воспроизведение ждёт всех и
  запускается одновременно
- хост может передать комнату другому участнику: тот поднимает её у себя, а
  остальные, включая старого хоста, переподключаются автоматически
- список участников с отметками кто ведёт и у кого идёт загрузка
- имя в комнате задаётся там же в настройках
- плавающая плашка "хостите / слушаете" поверх окна; её можно перетаскивать
  или совсем выключить в настройках, комната при этом продолжает работать
- журнал комнаты в настройках плюс лог-файл рядом с данными приложения
- по сети идут только команды: трек и позиция, звук каждый грузит из своего
  аккаунта, так что подписка нужна каждому своя
...
слушать вместе, горячие клавиши, текст песни, genius, эквалайзер, загрузки,
discord, кеш, о проекте.
...
desktop/
  ...
  src/
    ...
    components/         компоненты интерфейса
      ...
      together/         карточки комнаты и список участников
    ...
    stores/             pinia: плеер, библиотека, интерфейс, genius, статистика, таймер сна, комната
  src-tauri/
    icons/              иконки приложения
    src/
      commands/         команды, доступные из фронтенда
      genius/           апи genius: поиск, артист, песня, тексты
      together/         совместное прослушивание: комната, хост, гость, протокол
...
совместное прослушивание работает на своём tcp-сервере внутри клиента: хост
рассылает участникам трек, паузу и позицию строками json, гости применяют их
к своему плееру. внешних зависимостей у этой части нет.
...
| mashiro.hiddenPlaylists                      | спис��к скрытых плейлистов               |
| mashiro.together.nick                        | имя в комнате совместного прослушивания |
...
- genius: разборы строчек, авторы и продюсеры
